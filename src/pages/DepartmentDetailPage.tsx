    ...
    <div className="min-h-screen bg-gold">
      ...
      <section className="relative pt-32 pb-24 bg-indigo-950 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-12 relative z-10">
          <Link 
            to="/departments" 
            className="inline-flex items-center space-x-2 text-gold hover:text-gray-300 transition-colors mb-8 group"
          >
            <ArrowLeft size={18} className="group-hover:-translate-x-1 transition-transform" />
            <span className="text-[0.7rem] font-boldtext-gold uppercase tracking-widest">Back to All Departments</span>
          </Link>
          ...
          <motion.div
            ...
            className="max-w-3xl"
          >
            <span className="inline-block px-3 py-1 bg-gold/20 text-gold text-[0.7rem] font-bold uppercase tracking-widest rounded-sm mb-6 border border-gold/30">
              {department.category}
            </span>
            <h1 className="text-4xl md:text-5xl lg:text-7xl font-serif font-bold text-gold mb-6 leading-tight">
              {department.name}
            </h1>
            ...
            <div className="flex flex-wrap gap-6 mt-10">
              <div className="flex items-center space-x-3">
                <div className="w-12 h-12 rounded bg-gold/10 flex items-center justify-center text-gold backdrop-blur-sm">
                  <Users size={24} />
                </div>
                <div>
                  <div className="text-[0.6rem] uppercase tracking-widest text-gray-400 font-bold mb-0.5">Head of Dept</div>
                  <div className="text-gold font-medium">{department.hod}</div>
                </div>
              </div>
              <button className="bg-gray-800 text-gold px-8 py-4 rounded-sm font-bold text-sm tracking-widest uppercase hover:bg-gold hover:text-black transition-all transform hover:-translate-y-1 shadow-xl shadow-black/20">
                Apply to Department
              </button>
            </div>
          </motion.div>
      ...
      <section className="py-24 max-w-7xl mx-auto px-6 lg:px-12">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="relative">
            ...
            <div className="absolute -bottom-10 -left-10 w-64 h-64 bg-blue-950/5 -z-10 rounded-sm"></div>
            
            <div className="absolute top-1/2 -right-12 translate-y-1/2 bg-gold p-8 shadow-xl max-w-[240px] z-20 border-t-4 border-gold hidden xl:block">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6">
            {department.focusAreas.map((area, idx) => (
              <motion.div
                key={idx}
                whileHover={{ y: -5 }}
                className="bg-blue-950/5 backdrop-blur-sm border border-gold/10 p-8 rounded-sm text-center hover:bg-gold/10 transition-all"
              >
          ...
          <div className="max-w-7xl mx-auto px-6 lg:px-12">
            ...
            <div className="grid md:grid-cols-2 gap-10">
              {department.labs.map((lab, idx) => (
                <div key={idx} className="group bg-gold rounded-sm overflow-hidden shadow-md flex flex-col md:flex-row border border-gray-100">
                ...
                <div className="absolute bottom-4 left-4 right-4">
                  <div className="bg-blue-950/90 backdrop-blur-md p-4 translate-y-full group-hover:translate-y-0 transition-transform duration-300">
                    <div className="flex items-center space-x-3 text-gold">
                      ...
                      <span className="text-[0.6rem] font-bold uppercase tracking-widest hover:text-gold cursor-pointer">Contact Faculty</span>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <section className="py-24 bg-blue-950 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-12 relative z-10">
          <div className="grid lg:grid-cols-2 gap-20">
            {/* Achievements */}
            <div>
              <h3 className="text-2xl font-serif font-bold text-gold mb-10 flex items-center">
                <Trophy size={28} className="text-gold mr-4" /> Department Achievements
              </h3>
              <div className="space-y-6">
                {department.achievements.map((item, idx) => (
                  <div key={idx} className="flex items-start space-x-4 bg-gold/5 p-6 border-l-4 border-gold">
                    <div className="text-gold font-bold text-lg">0{idx + 1}</div>
                    <p className="text-gold/80 text-sm leading-relaxed">{item}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {department.facilities.map((item, idx) => (
                  <div key={idx} className="flex items-center space-x-3 bg-gold/5 px-6 py-4 rounded-sm border border-gold/10 hover:border-gold/50 transition-colors">
                    <div className="w-2 h-2 rounded-full bg-gold"></div>
                    <span className="text-gold text-[0.7rem] font-bold uppercase tracking-wider">{item}</span>
                  </div>
                ))}
              </div>
              <div className="mt-12 p-8 bg-gold rounded-sm">
                <h4 className="text-gold font-bold text-xl mb-4">Want to learn more?</h4>
                <p className="text-gold/80 text-sm mb-6">Download our detailed department brochure for program details, curriculum, and placement stats.</p>
                <button className="w-full bg-gray-800 text-gold font-bold py-3 uppercase tracking-widest text-xs hover:bg-blue-950/90 transition-colors">
      ...
      {/* CTA Section */}
      <section className="py-32 bg-gold text-center">
        <div className="max-w-3xl mx-auto px-6">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
            <button className="w-full sm:w-auto bg-gray-800 text-gold px-10 py-5 rounded-sm font-bold text-sm tracking-widest uppercase hover:bg-gold hover:text-navy transition-all shadow-xl shadow-navy/10">
              Apply for Admission
            </button>
            <button className="w-full sm:w-auto border-2 border-navy text-navy px-10 py-5 rounded-sm font-bold text-sm tracking-widest uppercase hover:bg-gray-800 hover:text-gold transition-all">
              Contact Department
            </button>
          </div>
        </div>
